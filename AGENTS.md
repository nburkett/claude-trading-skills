...
This repository contains Codex Skills for equity investors and traders. Each skill packages domain-specific prompts, knowledge bases, and helper scripts to assist with market analysis, technical charting, economic calendar monitoring, and trading strategy development. Skills are designed to work in both Codex's web app and Codex environments.
...
├── references/           # Knowledge bases loaded into Codex's context
...
- All instructions assume Codex will execute them, not the user
...
- Knowledge bases provide context without requiring Codex to have specialized training
...
- References (`references/`) are documentation for Codex to read and apply
...
Use the skill-creator plugin (available in Codex):
...
Skills are packaged as ZIP files for Codex web app users:

```bash
# Use the skill-creator's packaging script
python3 ~/.Codex/plugins/marketplaces/anthropic-agent-skills/skills/skill-creator/scripts/package_skill.py <skill-name>
```
...
Skills are tested by invoking them in Codex conversations:

1. Copy skill folder to Codex Skills directory
2. Restart Codex to detect the skill
...
# Configure Alpaca MCP Server in Codex settings
...
# Portfolio analysis is done via Codex with Alpaca MCP tools
...
**Architecture:**
- `scripts/run_skill_improvement_loop.py` — orchestrator (round-robin selection, auto scoring, Codex CLI improvement, quality gate, PR creation)
...
- Write instructions for Codex to execute, not user instructions
- Avoid phrases like "You should..." or "Codex will..." - just state actions directly
...
1. Test skill thoroughly in Codex
...
ZIP packages allow Codex web app users to upload and use skills without cloning the repository.
